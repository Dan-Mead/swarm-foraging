		...
		for i = 1,num_received_broadcasts do
			...
			--if base_origin ~= true then
				if robot.range_and_bearing[i].data[1] ~= 0 then
					if temp_base_time > broadcast_info.base.time then
						...
						broadcast_info.base.origin = false
			...
			if robot.range_and_bearing[i].data[6] ~= 0 then
				if temp_target_time > broadcast_info.target.time then
					...
					broadcast_info.target.origin = false
...
function data_conversion()
	...
	if broadcast_info.base.distance ~= math.huge then
		broadcast_data[1] = 1
	end
	
	if broadcast_info.target.distance ~= math.huge then
		broadcast_data[6] = 1
	end	
...
function data_broadcast()
	...
	if counter % broadcast_counter_reset == 0 then
